...
global{
	...
	int largo_mundo <- 1000;
	int ancho_mundo <- 1;
	int num_carros_vecinos <- 8; // Solo se puede utililzar 4, 6 u 8 y solo toma distancia de r=1
	int num_ciclos <- 1000; //int(largo_mundo/2) + 1;
	int cont_ciclos <- 0;
	
	float prob_densidad <- 0.9;
	int num_coches <- int(prob_densidad*largo_mundo);
	int interacciones_totales <-num_coches * num_ciclos;
	int conteo_interacciones <- 0; 
	...
	reflex when: cont_ciclos >= num_ciclos{
		write "interacciones_totales: " + interacciones_totales;
		write "conteo_interacciones: " + conteo_interacciones;
		float rendimiento <- conteo_interacciones / interacciones_totales; 
...
species coche skills: [moving]{
	...
	reflex actualizar_percepcion{
		...
		if celda_siguiente.celda_ocupada or celda_siguiente.estaba_ocupada{
			...
			conteo_interacciones <- conteo_interacciones + 1;
		}
	}
	reflex avanzar{
